...
def start_fight(hero: list) -> None:
    ...
    [10] inventory - список предметов
    """
    enemy = make_hero(hp_now=20, xp_now=30, money=50, inventory=["меч Гладиатора", "шапка Мономаха", "кольчуга гладиатора"])
    text = "Выберите вариант!"
    options = [
        "Ударить врага",
        "Использовать(употребить) предмет"
    ]
...
def visit_arena(hero: list):
    text = f"{hero[0]} добрался до арены."
    options = [
    ...
    if option == 0:
        ...
        input("Выберите правильный вариант.")
        return visit_arena(hero)
